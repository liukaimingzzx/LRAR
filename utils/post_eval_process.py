...
def parse_dataset_files(object_name, dataset_base_dir, anomaly_maps_dir, dataset="MVTec"):
    ...
    for subdir in anomaly_types:
        ...
        # MVTec/BUSI -> .png, VisA -> .JPG, BraTS -> .jpg
        if dataset in ("MVTec", "BUSI"):
            expected_ext = '.png'
        elif dataset == "BraTS":
            expected_ext = '.jpg'
        else:
        ...
        test_images = [path.splitext(file)[0]
                       ...
                       if path.splitext(file)[1] == expected_ext]

        # If subdir is not 'good', derive corresponding GT names.
        # MVTec/BUSI/BraTS -> <file>_mask.png, VisA -> <file>.png
        if subdir != 'good':
            if dataset in ("MVTec", "BUSI", "BraTS"):
                gt_filenames.extend(
...
def eval_segmentation(object_name, gt_filenames, prediction_filenames, pro_integration_limit=0.3, delete_tiff_files=True, dataset=None):
    ...
    # For datasets with variable image sizes (e.g., BUSI),
    # resize all GT and predictions to the same shape (the first image's shape)
    # so that compute_pro can stack them into a 3D array.
    ref_shape = predictions[0].shape
    need_resize = any(p.shape != ref_shape for p in predictions) or \
                  any(g.shape != ref_shape for g in ground_truth)
    if need_resize:
        resized_predictions = []
        resized_ground_truth = []
        for pred, gt in zip(predictions, ground_truth):
            if pred.shape != ref_shape:
                pred = cv2.resize(pred.astype(np.float32), (ref_shape[1], ref_shape[0]),
                                  interpolation=cv2.INTER_LINEAR)
            resized_predictions.append(pred)
            if gt.shape != ref_shape:
                gt = cv2.resize(gt.astype(np.uint8), (ref_shape[1], ref_shape[0]),
                                interpolation=cv2.INTER_NEAREST)
            resized_ground_truth.append(gt)
        predictions = resized_predictions
        ground_truth = resized_ground_truth
...
def eval_finished_run(dataset, dataset_base_dir, anomaly_maps_dir, output_dir, seed = None, pro_integration_limit = 0.3, eval_clf = True, eval_segm = False, delete_tiff_files = False, aggregation_statistics = "meantop1p"):
    ...
    with ThreadPoolExecutor(max_workers=len(objects)) as executor:
        ...
        for obj, result in results:
            ...
            if result is None:
                evaluation_dict[obj] = {'error': 'Evaluation failed, see logs above'}
                print(f"  Evaluation failed for {obj}, skipping.")
                continue
