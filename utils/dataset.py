...
class FSDataset(Dataset):
    def __init__(self,
        ...
        if data_mode == 'mvtec_visa':
            data = ['mvtec', 'visa']
        elif data_mode == 'mvtec_busi':
            data = ['mvtec', 'BUSI/Breast']
        elif data_mode == 'mvtec_brats':
            data = ['mvtec', 'brats']
        else:
            raise ValueError(f"Unsupported data_mode: {data_mode}. Expected 'mvtec_visa', 'mvtec_busi' or 'mvtec_brats'.")
    ...
    def build_class_ids(self):
        ...
        elif self.data_mode == 'mvtec_busi':  # mvtec and busi
            # mvtec has 15 products (0-14), busi has 1 product "Breast" (15)
            # fold=0: val on MVTec(0-14), train on BUSI(15)  -> BUSI->MVTec
            # fold=1: val on BUSI(15), train on MVTec(0-14)  -> MVTec->BUSI
            class_ids_val = range(0, 15) if self.fold == 0 else range(15, self.nclass)
            class_ids_trn = [x for x in range(self.nclass) if x not in class_ids_val]
        elif self.data_mode == 'mvtec_brats':  # mvtec and brats
            # mvtec has 15 products (0-14), brats has 1 product "brain" (15)
            # fold=0: val on MVTec(0-14), train on BraTS(15)  -> BraTS->MVTec
            # fold=1: val on BraTS(15), train on MVTec(0-14)  -> MVTec->BraTS
            class_ids_val = range(0, 15) if self.fold == 0 else range(15, self.nclass)
            class_ids_trn = [x for x in range(self.nclass) if x not in class_ids_val]
        else:
            raise ValueError(f"Unsupported data_mode: {self.data_mode}. Expected 'mvtec_visa', 'mvtec_busi' or 'mvtec_brats'.")
