...
def get_ref_sample_dir(data_root, object_name, mode, type_anomaly, ref_samples, seed):
    """
    循环索引选择 reference，避免 seed 导致切片为空。

    原来的写法:
        samples[seed*ref_samples:(seed+1)*ref_samples]

    当 seed 较大、样本数量较少时，可能直接返回空列表。
    这里改成:
        samples[(seed + k) % len(samples)]
    从而保证只要目录里有样本，就一定能取到 reference。
    """
    img_ref_folder = f"{data_root}/{object_name}/{mode}/{type_anomaly}"
    all_samples = sorted(glob.glob(os.path.join(img_ref_folder, "*")))

    if len(all_samples) == 0:
        print(f"Warning: No reference samples found in {img_ref_folder}")
        return []
    ...
    selected_samples = []
    n = len(all_samples)
    for k in range(ref_samples):
        idx = (seed + k) % n
        selected_samples.append(all_samples[idx])
...
def run_anomaly_detection(
    ...
    normal_ref_dir = get_ref_sample_dir(
        data_root, object_name, 'train', NORMAL_NAME, args.n_shot, seed
    )
    ...
    if len(normal_ref) == 0:
        raise RuntimeError(
            f"No normal references selected for object={object_name}, "
            f"seed={seed}, n_shot={args.n_shot}"
        )
    ...
    # 固定随机数生成器，保证每个 seed 下 NORMAL_NAME 分支选取异常类型可复现
    ...
    for type_anomaly in tqdm(type_anomalies, desc=f"processing test samples ({object_name})"):
        ...
        if args.a_shot > 0:
            if type_anomaly == NORMAL_NAME:
                if len(original_type_anomalies) == 0:
                    raise RuntimeError(
                        f"No abnormal anomaly types available for object={object_name}, "
                        f"cannot sample abnormal references for NORMAL test samples."
                    )
                sampled_anomaly = rng.choice(original_type_anomalies)
                abnormal_ref_dir = get_ref_sample_dir(
                    data_root, object_name, 'test', sampled_anomaly, args.a_shot, seed
                )
            else:
                abnormal_ref_dir = get_ref_sample_dir(
                    data_root, object_name, 'test', type_anomaly, args.a_shot, seed
                )
            ...
            for ref_dir in abnormal_ref_dir:
                ...
                # Build GT mask path from image path.
                # MVTec/BUSI: xxx.png -> xxx_mask.png
                # VisA:      xxx.JPG -> xxx.png
                # BraTS:     xxx.jpg -> xxx_mask.png
                # NOTE: order matters to avoid double-replacement (e.g. '.jpg' -> '_mask.png'
                # then '.png' -> '_mask.png' would produce '_mask_mask.png').
                if ref_dir.endswith('.JPG'):
                    mask_path = ref_dir.replace('test', 'ground_truth').replace('.JPG', '.png')
                elif ref_dir.endswith('.jpg'):
                    mask_path = ref_dir.replace('test', 'ground_truth').replace('.jpg', '_mask.png')
                else:
                    mask_path = ref_dir.replace('test', 'ground_truth').replace('.png', '_mask.png')

                mask_img = cv2.imread(mask_path, cv2.IMREAD_COLOR)
                if mask_img is None:
                    raise RuntimeError(f"Failed to read abnormal reference mask: {mask_path}")

                mask_ref = cv2.cvtColor(mask_img, cv2.COLOR_BGR2GRAY)
            ...
            if len(abnormal_ref) == 0:
                raise RuntimeError(
                    f"abnormal_ref is empty before stack. "
                    f"object={object_name}, type_anomaly={type_anomaly}, "
                    f"seed={seed}, a_shot={args.a_shot}, abnormal_ref_dir={abnormal_ref_dir}"
                )
